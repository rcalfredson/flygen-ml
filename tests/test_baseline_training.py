...
def test_train_and_save_run_writes_artifacts(tmp_path):
    ...
    features_path.write_text(
        "\n".join(
            [
                "fly_id,sample_key,genotype,chamber_type,training_idx,n_segments,n_segments_with_qc_flags,duration_frames_mean,path_length_px_mean,straightness_mean",
                "a0,s0,A,large,1,3,0,10.0,1.0,0.9",
                "a1,s1,A,large,1,3,0,11.0,1.2,0.8",
                "a2,s2,A,large,1,3,0,9.5,0.8,0.85",
                "b0,s3,B,large,1,3,0,20.0,5.0,0.2",
                "b1,s4,B,large,1,3,0,19.0,4.8,0.3",
                "b2,s5,B,large,1,3,0,21.0,5.2,0.25",
            ]
        )
    )
...
def test_train_and_save_cross_validation_run_writes_fold_artifacts(tmp_path):
    ...
    features_path.write_text(
        "\n".join(
            [
                "fly_id,sample_key,genotype,chamber_type,training_idx,n_segments,n_segments_with_qc_flags,duration_frames_mean,path_length_px_mean,straightness_mean",
                "a0,s0,A,large,1,60,0,10.0,1.0,0.9",
                "a1,s1,A,large,1,30,0,11.0,1.2,0.8",
                "a2,s2,A,large,1,5,0,9.5,0.8,0.85",
                "b0,s3,B,large,1,60,0,20.0,5.0,0.2",
                "b1,s4,B,large,1,30,0,19.0,4.8,0.3",
                "b2,s5,B,large,1,5,0,21.0,5.2,0.25",
            ]
        )
    )
    config_path = tmp_path / "logreg.yaml"
